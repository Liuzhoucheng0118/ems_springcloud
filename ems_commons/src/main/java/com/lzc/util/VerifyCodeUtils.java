...
public class VerifyCodeUtils {
    ...
    // 生成随机数的源
    private final static String VERIFY_CODE_SRC = "0123456789";
     ...
     * @return 验证码
     */
    public static String generateVerifyCode(int width, int height, String code, OutputStream outputStream) {
        ...
        // 设置边框颜色
        Color randomColor = VerifyCodeUtils.getRandomColor(100, 150);
        graphics2D.setColor(Color.GRAY);
        graphics2D.fillRect(0, 0, width, height);

        // 设置背景颜色
        randomColor = VerifyCodeUtils.getRandomColor(200, 250);
        graphics2D.setColor(randomColor);
        graphics2D.fillRect(0, 2, width, height - 3);

        // 设置字体
        graphics2D.setColor(VerifyCodeUtils.getRandomColor(100, 150));
        graphics2D.setFont(new Font("Consolas", Font.ITALIC, (int) (height * 0.9)));

        //绘制干扰线
        VerifyCodeUtils.drawInterferenceLine(graphics2D, width, height);

        // 添加噪点
        VerifyCodeUtils.drawNoise(bufferedImage, width, height);

        // 扭曲图片
        VerifyCodeUtils.twist(graphics2D, width, height, randomColor);

        // 写入验证码
        VerifyCodeUtils.writeVerifyCode(graphics2D, verifyCode, width, height);
    ...
    private static void writeVerifyCode(Graphics2D graphics2D, String verifyCode, int width, int height) {
        ...
        for (int i = 0; i < verifyCode.length(); i++) {
            // 为每个验证码指定不同的颜色
            graphics2D.setColor(VerifyCodeUtils.getRandomColor(80, 150));

            // 旋转验证码
            // FIXME 旋转后的验证码辨识度太低，待完善
...
//            graphics2D.setTransform(affineTransform);
    ...
    private static void drawNoise(BufferedImage image, int width, int height) {
        ...
        for (int i = 0; i < area; i++) {
            ...
            int rgb = VerifyCodeUtils.getRandomIntColor();
    ...
    private static void drawInterferenceLine(Graphics2D graphics2D, int width, int height) {
        // 设置线条的颜色
        graphics2D.setColor(VerifyCodeUtils.getRandomColor(160, 200));
        // 20条干扰线
        for (int i = 0; i < 20; i++) {
    ...
    public static String generateVerifyCode(int verifyCodeSize) {
        // 生成 verifyCodeSize 个随机数
        String verifyCode = VerifyCodeUtils.getRandomVerifyCode(verifyCodeSize, VERIFY_CODE_SRC);
